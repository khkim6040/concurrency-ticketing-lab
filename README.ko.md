...
하려는 말은 하나입니다. 정합성을 강하게 걸수록 처리량은 줄어듭니다. 두 축을 늘 함께 띄웁니다. 방어를 전부 켜는 것이 정답인 도구라면 잘못된 것을 가르치는 셈이기 때문입니다.

![같은 시드로 두 번 실행: oversell NONE은 실패하고, CONDITIONAL_UPDATE + 유니크 인덱스는 통과합니다. 둘을 나란히 비교합니다](docs/demo.gif)

## 무엇을 하는가

- 앱 서버 두 대, MySQL, Redis를 Docker Compose로 띄웁니다. 한 대면 JVM 락 하나로 여기 있는 문제가 전부 풀려 버리고, 그것이야말로 배우면 안 되는 결론이라 두 대가 최소입니다.
- `raceWindowMs` 손잡이는 읽기와 쓰기 사이, 락이나 트랜잭션 안에서 잠듭니다. 덕분에 경쟁이 운 좋은 몇 번이 아니라 매 실행 재현됩니다.
...
- 실행이 끝난 뒤 DB에서 세는 값 넷: `oversoldCount`, `ledgerMismatch`, `doubleBookedSeats`, `duplicateKeyCount`. 가는 길에 재는 값 넷: 처리량, p50/p95/p99, `retryCount`, `dbConnectionPeak`. 판정은 PASS 아니면 FAIL이고, 정합한 실행이 같은 조건 `NONE`의 절반 이하 처리량이면 DEGRADED입니다.
- 조회자 스레드 두 개가 실행 중과 종료 후 2초 동안 10ms마다 재고 캐시(`GET /api/stock`)를 찌릅니다. 매진 뒤에도 잔여석이 있다고 답한 조회가 `phantomStockViews`이고, `staleWindowMs`·`viewDbReads`와 함께 보고합니다. 셋 다 판정에는 들어가지 않습니다. stale read는 설계 선택이고, UI도 보고서 아래에 그렇게 적습니다.
...
읽어야 할 것은 `LOCAL_LOCK` 두 줄입니다. 앱 한 대에서는 락이 모든 요청을 줄 세워 카운터가 정확히 맞습니다. 정원 초과도 원장도 0입니다. 두 대에서는 각 JVM이 자기 절반만 줄 세우고 두 절반이 서로 경쟁해, 좌석 100개 중 94\~100개를 초과 판매합니다. 나머지 전략을 앱 2대로만 적은 이유는 방어가 DB에 있어 1대 숫자가 노이즈 범위에서 같기 때문입니다. 모든 줄이 FAIL인 것은 여기서 `doubleBooking`이 `NONE`이라 카운터 총합이 정확해도 한 좌석이 두 사람에게 갈 수 있어서입니다. 그 축은 다음 표가 따로 떼어냅니다. `PESSIMISTIC`이 커넥션 최대치를 풀 크기에 붙여 놓는 것은 각 트랜잭션이 행 락 안에서 자는 동안 커넥션을 쥐고 있기 때문입니다. `OPTIMISTIC`은 카운터를 정확히 지키면서 두 락보다 빠르지만, 그 값으로 수만 번의 재시도를 치릅니다.
...
`CONDITIONAL_UPDATE`만으로는 개수는 맞지만 좌석이 겹쳐 여전히 실패합니다. 위층 코드가 무엇을 했든 두 번째 예약을 거절하는 층은 유니크 인덱스뿐입니다.

반대는 성립하지 않습니다. 유니크 인덱스는 많아야 N개의 요청만 카운터까지 보내므로, `UNIQUE_CONSTRAINT`에서는 oversell 전략이 무엇이든 `oversoldCount`가 0입니다. `oversell=NONE`에 `doubleBooking=UNIQUE_CONSTRAINT`를 걸어 보고 `oversoldCount`가 아니라 `ledgerMismatch`를 읽으시면 됩니다. 또 저 줄의 `PASS`는 정합하다는 뜻일 뿐입니다. DoD 스크립트가 그 좌석 전략의 `NONE` 기준선을 남기지 않아 거기서는 `DEGRADED` 검사가 아예 돌지 않습니다.
...
`NONE`은 처음 본 값을 TTL이 끝날 때까지 쥐고 있는데, 그 시점은 실행이 끝난 뒤입니다. `TTL_SHORT`는 창을 TTL만큼으로 묶을 뿐 그 아래로는 못 내립니다. 창을 0으로 닫는 TTL은 캐시가 없는 것과 같습니다. `INVALIDATE_ON_WRITE`가 옳은 것은 캐시를 놓쳐 DB를 읽고 잠들었던 조회자가 마지막 쓰기의 삭제 뒤에 자기 옛값을 얹기 전까지입니다. 그다음에는 아무도 다시 지우지 않습니다. 기본 20ms 창에서는 5번 중 2번, 200ms에서는 다섯 번 모두 걸립니다. 이 경쟁은 읽기 지연과 쓰기 폭주의 관계가 만드는 성질이고, 손잡이는 그것을 보이게 할 뿐입니다. `REDIS_AS_SOT`는 두 번째 복사본이 없어 측정값이 0입니다. 대가는 DB 조회 열에 있습니다. 창이 짧을수록 캐시가 덜 흡수합니다.

### 모든 조합

아래 모든 경우는 N=100, M=1,000, `raceWindowMs=20`, `seed=42`이고, 캡션에 달리 적지 않으면 앱 2대입니다. `./scripts/case-shots.sh`가 찍습니다. 경우마다 스크린샷 한 장에 좌석 그리드, 판정, 그 실행의 지표 표 전체가 들어 있습니다. FAIL 25, PASS 7, DEGRADED 6.

축 세 개와 앱 대수로 조합은 80가지지만 38가지로 접힙니다. `REDIS_AS_SOT`는 카운터를 Redis에 두고 oversell 전략을 무시하므로 다섯 줄이 한 줄이 됩니다. `appInstances`가 결과를 바꾸는 전략은 JVM 안에 사는 유일한 방어인 `LOCAL_LOCK`뿐입니다. 나머지는 MySQL이나 Redis에 있어 앱 1대와 2대가 같게 읽힙니다.

각 묶음은 `oversell=NONE`을 먼저 돌리므로 앱 2대에서는 `DEGRADED` 검사에 기준선이 있습니다. 앱 1대 실행은 맞는 기준선이 없어 PASS 아니면 FAIL만 나옵니다. `LOCAL_LOCK` 1대가 `PESSIMISTIC`이라면 DEGRADED로 찍힐 처리량에서 PASS로 나오는 이유입니다.
...
`./gradlew test`가 단위 테스트를, `node --test src/test/js/ui.test.mjs`가 UI 함수를 돌립니다. 둘 다 Docker 없이 돕니다.
...
부하 생성기는 가상 스레드 M개를 띄워 `CountDownLatch` 하나 뒤에 줄 세웁니다. 요청은 브라우저가 아니라 서버에서 출발해야 합니다. 그러지 않으면 출발 시각이 흔들리는 것만으로 경쟁이 사라집니다.

DB 접근은 `JdbcClient`로 하고 SQL은 손으로 씁니다. JPA는 일부러 쓰지 않았습니다. 락 층(`FOR UPDATE`, 조건부 업데이트, 버전 확인)이 코드에 보이지 않으면 배울 것이 없습니다. Redis는 `stock:{eventId}`에 재고 캐시를 들고, `REDIS_AS_SOT`에서는 카운터 자체를 듭니다.
...
- [결정 기록](docs/decisions.md). 스펙이 열어둔 선택지와 코드가 스펙과 달리 간 부분을 한 줄에 하나씩 적었습니다.
