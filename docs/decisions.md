...
| 2026-09-21 | 해설은 라디오 옆 `<details>`, 영어 기본 + 한국어 토글(`<html lang>` + `localStorage`). 링크에 언어 없음 | 저장소 방문자 기준 영어. 결과 직후 고른 전략 3개를 자동 펼침(이미 열린 것은 그대로). |
...
| 2026-09-21 | 비교 표는 파라미터 8개를 본문 행으로 두고 지표 행과 같은 직전/이번/차이 열을 쓴다. 스펙의 머리 두 행 대신 | 열 구조가 하나라 표가 단순하고, 달라진 파라미터는 행 전체를 굵게 하면 충분하다. |
| 2026-09-22 | 데모 GIF의 커서는 `addInitScript`로 페이지에 심은 점이고, 클릭 전에 `mouse.move(..., steps)`로 끌고 간다 | 헤드리스 `recordVideo`에는 OS 커서가 찍히지 않는다. `click`만 쓰면 커서가 순간이동해 어디를 눌렀는지 영상에 남지 않는다. |
| 2026-09-22 | 마일스톤 설계·계획 문서는 `docs/milestones/`에 두고 README는 그 디렉터리만 한 줄로 링크한다. 전략별 근거는 그것을 증명하는 표 해설에만 둔다 | 저장소 방문자에게 보일 이름은 쓰인 도구가 아니라 내용이다. 기능 불릿마다 근거를 반복하면 README가 이 결정 기록과 겹치고 길어지기만 한다. |
| 2026-09-22 | 조합별 결과는 GIF가 아니라 최종 상태 PNG 한 장(`scripts/case-shots.sh`). 움직이는 데모는 비교 GIF 한 편만 남긴다 | 옵션별 결과는 거의 결정적이라 움직임이 정보를 더하지 않는다. 접은 `<details>` 안의 이미지도 브라우저가 내려받으므로 38편 GIF는 README를 한 번 열 때 30\~72MB이고, PNG는 2.8MB다. |
| 2026-09-22 | 조합 80가지를 38가지로 접는다. `REDIS_AS_SOT`는 oversell을 무시하고, `appInstances`는 `LOCAL_LOCK`에서만 결과를 바꾼다 | 나머지 방어는 MySQL·Redis에 있어 앱 1대와 2대가 같다. 접지 않으면 같은 그림이 42장 늘어난다. |
| 2026-09-22 | 한국어 README는 루트의 `README.ko.md` 한 벌이고, 두 파일 제목 아래 상호 링크 한 줄을 둔다 | GitHub에 README 다국어 기능이 없어 `README.md`만 렌더링된다. 전략 이름·파라미터·지표 키·판정 단어를 번역하지 않는 규칙 덕분에 조합 스크린샷 블록은 요약 6줄만 바꿔 그대로 쓴다. |
| 2026-09-22 | 한국어 README는 합쇼체(\~합니다)로 쓴다. `-는가` 형 섹션 제목과 파라미터 이름·지표 키는 그대로 둔다 | 저장소 방문자에게 하는 설명이라 해라체보다 합쇼체가 맞다. 제목의 `-는가`는 서술이 아니라 표제라 어느 문체에서도 중립이고, 고유명사 번역 금지 규칙은 그대로다. |
| 2026-09-22 | 마크다운 본문의 범위 물결표는 모두 `\~`로 이스케이프한다 | GFM은 물결표 하나짜리도 취소선으로 처리해서, 한 셀이나 한 줄에 `~`가 둘 있으면 쌍이 맺힌다. README 표의 1\~2 / 267\~294가 취소선으로 렌더링됐다. 셀당 하나면 안전하지만 그 불변식은 다음 편집에서 깨지므로 전부 이스케이프한다. |
| 2026-09-22 | UI 한국어 문자열도 README와 같은 합쇼체로 맞추고, `이(가)` 괄호 조사는 목록을 문장 뒤로 빼서 없앤다 | 같은 저장소를 읽고 온 사람에게 문체가 튀면 안 된다. 괄호 조사는 영문 식별자 뒤 조사를 고를 수 없어 생긴 i18n 흔적이라, `FAIL: 0이 아닌 지표 oversoldCount`처럼 목록을 뒤로 빼면 조사 자체가 필요 없다. |
